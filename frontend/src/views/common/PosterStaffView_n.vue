    ...
    <a-row>
      ...
      <a-col :span="15" :offset="1">
        ...
        <a-popconfirm
          ...
          cancelText="取消"
        >
          <a-button type="primary">批量面试</a-button>
        </a-popconfirm>
        <a-popconfirm
          title="确定批量通过？"
      ...
      if (this.selectedRowKeys.length > 0) {
        this.$put('zpStaffInfo/applyStates', {
          ...
          if (r.data.data.success === 1) {
            this.openNotificationIcon('success', '操作提醒', '批量面试简历成功.')
            this.$emit('success')
            this.search()
          } else {
            this.openNotificationIcon('error', '操作提醒', '批量面试简历失败.')
          }
          this.loading = false
        }).catch(() => {
          this.loading = false
        ...
        this.openNotificationIcon('warning', '操作提醒', '批量面试, 未选择项.')
      }
    },
    batchTg () {
      if (this.selectedRowKeys.length > 0) {
    ...
    batchJj () {
      if (this.selectedRowKeys.length > 0) {
        this.$put('zpStaffInfo/applyStates', {
          ...
          if (r.data.data.success === 1) {
            this.openNotificationIcon('success', '操作提醒', '批量拒绝简历成功.')
            this.$emit('success')
            this.search()
          } else {
            this.openNotificationIcon('error', '操作提醒', '批量拒绝简历失败.')
          }
          this.loading = false
        }).catch(() => {
          this.loading = false
        ...
        // this.$message.warning('未选择项.')
        this.openNotificationIcon('warning', '操作提醒', '批量拒绝, 未选择项.')
      }
    },
